...
下载地址（将 `<version>` 替换为实际版本号，如 `0.1.7`）：
...
需要自动获取最新版本号时，`https://datalens.databuff.com:5001/databuff/VERSION` 仅返回纯文本版本（如 `0.1.7`），**不是**离线包地址。可先查版本再下载：
...
以 `0.1.7` / `amd64` 为例（在能联网的机器下载后，将离线包拷贝到目标机）：
