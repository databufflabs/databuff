...
安装时如何跳过 AVX2 检测，以及安装完成后如何修改登录账号、配置 Doris 连接账号与 FE/BE 地址、调整 Ingest 管道任务参数、配置 Trace resource 忽略过滤、SQL 标准化、调整遥测数据存储时长。目录与启停见 [Docker 运维](Docker运维.md)、[Kubernetes 运维](K8s运维.md)；容量与更多调优项见 [性能优化与容量规划](性能优化.md)。
