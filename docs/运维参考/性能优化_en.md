...
|----------|--------------------------------------|---------|---------|----------|----------|
| `ingest.doris.flush-batch-bytes` | `INGEST_DORIS_FLUSH_BATCH_BYTES` | `52428800` (50 MiB) | Hand off thread buffer for Stream Load at this estimated NDJSON size | **Prefer larger on single-BE / load tests**: fewer Stream Loads, stay under tablet version ~2000 | Too-small batches → version pile-up, compaction lag, BE RSS blow-up |
...
| `ingest.doris.flush-timeout-ms` | `INGEST_DORIS_FLUSH_TIMEOUT_MS` | `60000` | Stream Load wait cap (min 5000) | Tolerate slow BE / large 50 MiB batches | Fail faster on timeouts |
