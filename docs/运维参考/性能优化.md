...
|----------------|-------------------------|--------|------|------|------|
| `ingest.doris.flush-batch-bytes` | `INGEST_DORIS_FLUSH_BATCH_BYTES` | `33554432`（32 MiB） | 线程缓冲达到该估算 NDJSON 体积即 hand-off Stream Load | **压测/单 BE 优先调大**：降低 Stream Load 频率，避免 tablet version 打满 ~2000 | 过小会高频 load → version 堆积、compaction 跟不上、BE RSS 飙升 |
...
| `ingest.doris.flush-timeout-ms` | `INGEST_DORIS_FLUSH_TIMEOUT_MS` | `60000` | 单次 Stream Load 等待上限（最小 5000） | 容忍慢 BE / 大 32MiB 批次 | 超时失败更早暴露 |
