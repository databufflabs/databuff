...
适用：一键安装后的单机 Docker 部署（默认目录 `/opt/databuff-ai-apm`，可用 `echo $APM_INSTALL_DIR` 确认）。推荐用 `docker-compose.override.yml` 持久化自定义项（升级不覆盖），完整步骤含 K8s 见 [参数配置](参数配置.md#4-调整-ingest-任务参数)。
