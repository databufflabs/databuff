...
适用：一键安装后的单机 Docker 部署（默认目录 `/opt/databuff-ai-apm`，可用 `echo $APM_INSTALL_DIR` 确认）。推荐用 `docker-compose.override.yml` 持久化自定义项（升级不覆盖），完整步骤含 K8s 见 [参数配置](参数配置.md#_4-调整-ingest-任务参数)。
...
| 安装报 AVX2 / Doris 向量化 | `grep avx2 /proc/cpuinfo`；是否误跳过检测 | 见 [§7 x86_64 无 AVX2](#_7-x86-64-无-avx2-时绕过安装检测)；换 CPU 或 `DATABUFF_SKIP_AVX2_CHECK=1` |
