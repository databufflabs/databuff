...
Download URLs (replace `<version>` with the release, e.g. `0.1.8`):
...
Example CLI download (amd64 / `0.1.8`):

```bash
curl -fLO https://datalens.databuff.com:5001/databuff/0.1.8/offline/databuff-ai-apm-offline-0.1.8-amd64.tar.gz
```

To resolve the latest release automatically, `https://datalens.databuff.com:5001/databuff/VERSION` returns plain text (e.g. `0.1.8`) — **not** the bundle. Query the version first, then download:
...
Example for `0.1.8` / `amd64` (download on a connected machine, then copy the bundle to the target):

```bash
tar -zxvf databuff-ai-apm-offline-0.1.8-amd64.tar.gz
cd databuff-ai-apm-offline-0.1.8-amd64
