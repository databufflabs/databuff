 ...
 * objects.
 *
 * <p>DataBuff classpath override of agentscope-extensions-model-anthropic 2.0.0: non-streaming
 * tool_use {@code content} must be real JSON. Upstream used {@code JsonValue.toString()}, which for
 * {@code JsonObject} is Java {@code Map.toString()} (e.g. {@code {a=1}}), not JSON. That made
 * {@code ToolCallsAccumulator} replace content with {@code "{}"} while {@code input} stayed
 * correct, so {@code ToolExecutor} failed with {@code required property not found} (seen on
 * MiniMax Anthropic-compatible, stream=false).
 */
public class AnthropicResponseParser {
                            ...
                            toolUse -> {
                                ...
                                contentBlocks.add(
                                        ToolUseBlock.builder()
                                                ...
                                                .name(toolUse.name())
                                                .input(input)
                                                .content(jsonContent(toolUse._input(), input))
                                                .build());
                            });
    ...
    /**
     * Serialize tool arguments for {@link ToolUseBlock#getContent()}. Never use
     * {@link JsonValue#toString()} — {@code JsonObject.toString()} delegates to
     * {@code Map.toString()}, which is not JSON.
     */
    private static String jsonContent(JsonValue jsonValue, Map<String, Object> parsedInput) {
        if (jsonValue != null) {
            try {
                return ObjectMappers.jsonMapper().writeValueAsString(jsonValue);
            } catch (Exception e) {
                log.warn("Failed to serialize tool input JsonValue: {}", e.getMessage());
            }
        }
        ...
        try {
            return JsonUtils.getJsonCodec().toJson(parsedInput);
        } catch (Exception e) {
            log.warn("Failed to serialize parsed tool input: {}", e.getMessage());
            return "{}";
        }
    }
}
