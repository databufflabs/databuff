...
class ToolExecutor {
    ...
    private Mono<ToolResultBlock> executeCore(ToolCallParam param) {
        ...
        // Validate the same args that invocation will use. Upstream validated getContent() only;
        // Anthropic non-stream (MiniMax) often left content as Map.toString()/"{}" while input was
        // correct. resolveToolCallArgsJson also keeps "{}" when it is valid JSON — so prefer input.
        String validationError =
                ToolValidator.validateInput(
                        resolveValidationArgsJson(toolCall), tool.getParameters());
    ...
    /**
     * JSON used for schema validation. Prefer non-empty {@link ToolUseBlock#getInput()} (what
     * invocation uses); only fall back to content / resolveToolCallArgsJson when input is empty.
     */
    private static String resolveValidationArgsJson(ToolUseBlock toolCall) {
        ...
        return JsonUtils.resolveToolCallArgsJson(toolCall);
    }
}
