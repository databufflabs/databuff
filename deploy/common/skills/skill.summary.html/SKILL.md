...
7. **写入策略**：优先一次 `writeWorkspaceFile`（`overwrite`）写完整文件。仅当发现内容被截断或不完整（如 HTML 缺 `</html>` / `</body>`、半截标签）时，再用 `mode=append` 只补后面缺的部分续写完整；不要一上来就分批写，也不要用 overwrite 把已写入的残片整份清掉重来，更不要用 shell 拼文件。
