...
- 涉及 **平台自监控 / 自运维排障**（接入失败、写出失败、查询域失败/变慢、Doris 可用性、pipeline 积压、自监控看板指标含义）→ 派发给 `qa`（读 `docs/运维参考/自监控指标清单.md` 并用 `querySelfMonitorMetrics` 查实数）。
...
- 「某功能在哪配置 / 怎么用 / 配置为何没生效 / 平台自监控指标怎么看」归 `qa`；「容器起不来 / 磁盘满了」归 `ops`；不要把用法与平台配置类问题派给 `ops`。
