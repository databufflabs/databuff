...
#   tar -xzf databuff-ai-apm-offline-0.1.7-amd64.tar.gz
...
show_summary() {
  ...
  if command -v docker >/dev/null 2>&1; then
    mapped="$(docker port ai-apm-web 27403/tcp 2>/dev/null | head -n1 || true)"
    [ -n "$mapped" ] && web_port="${mapped##*:}"
    mapped="$(docker port ai-apm-ingest 4318/tcp 2>/dev/null | head -n1 || true)"
    [ -n "$mapped" ] && ingest_port="${mapped##*:}"
  fi
