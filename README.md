...
<h3>AI 原生 OpenTelemetry APM · 自愈运维闭环（Self-Healing Ops）</h3>
...
- 🔗 **SkyWalking 原生接入** — gRPC :11800，保留现有 Java Agent jar，将 `collector.backend_service` 指向 DataBuff 即可获取 traces / JVM 指标 / 拓扑 / 日志关联
- 🚨 **告警闭环** — 阈值与突变检测、定时评估、告警事件记录
- 🛠 **Ops Expert · SSH 故障排查** — 安装失败恢复与运行时根因排查，通过目标主机只读 SSH 巡检；内置 `expertId=ops`，受 shell 黑名单（`rm`、`dd`、`sudo`、`iptables` 等）约束，只读分诊不改写
- 🔄 **自愈运维闭环（Self-Healing Ops）** — 先观察再诊断：AI 巡检专家分诊异常并给出修复建议；自主 inspect → diagnose → remediate 是 Roadmap 上的长期愿景
...
**2. 升级已有 Docker 安装**（v0.1.3+，保留 `data/`）
...
> 最新版本：**[v0.1.3 — SkyWalking 原生接入 + Ops Expert SSH 故障排查](https://github.com/databufflabs/databuff/releases/tag/0.1.3)**
