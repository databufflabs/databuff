    ...
    <div class="alarm-wrapper flex-v">
      ...
      <div class="list-wrapper flex-1">
        <db-table
          ...
          tableKey='APM_RESOURCE_DETAIL_LOG'
          ref='listTable'>
        </db-table>
      </div>
    </div>
...
export default class TabLog extends Vue {
  ...
  private queryApi = ServiceApi.getResourceSpanList
  ...
  private async fetchTrendSource () {
    ...
    if (!error) {
      ...
      const allSource: any[] = timeKeyStr.map(i => +i).sort().map(i => {
      ...
      this.chartGroup.trend.source = [{
        ...
        name: i18n.t('modules.views.appMonitor.resourceDetail.s_ad548a23') as string, nameKey: 'modules.views.appMonitor.resourceDetail.s_ad548a23',
        type: 'bar',
        stack: 'call',
        data: allSource,
      }]
